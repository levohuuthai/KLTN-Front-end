...
function ListOrderDetailShipperAdmin(props) {
  ...
  useEffect(() => {
    const fetchRequestGetOrderDetailByID = async () => {
      try {
        const requestGetAllOrder = await orderAdminApi.getOrderById(
          dataOrderNavigate?._id
        );
        // console.log(requestGetAllOrder);
        setDataOrder(requestGetAllOrder.data);
      } catch (error) {
        console.log(error);
      }
    };
    fetchRequestGetOrderDetailByID();
  }, []);
  ...
  const [isOpenFormListShipper, setIsOpenFormListShipper] = useState(false);
  ...
  return (
    <>
      ...
      <FormUpdateStatusOrder
        ...
        idShipper={state.dataSelectShipper?._id}
      >
